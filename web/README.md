...
或未被撤销时，可以从此电脑部署三项资源，无需每次进入 Cloudflare 网页。
...
(cd Cloudflare/EmergencyUpdateWorker && \
  npx wrangler pages deploy ../../web/privacy --project-name privacy-policy --branch main)

# Universal Link 与网页跳转 Worker
(cd Cloudflare/EmergencyUpdateWorker && \
  npx wrangler deploy --config ../../web/open-worker/wrangler.jsonc)
