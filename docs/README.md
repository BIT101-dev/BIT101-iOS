...
- 统一课程检索、课程评价入口、学期同步和验证码续接。
- 优化课表周次、重叠课程、DDL、空教室、日历和课程地点导航。
- 支持可信成绩单与短期认证会话，改进登录和异常状态提示。
- 合并评论、信息流、搜索、头像、标签和状态组件，减少界面样式分叉。
- 完善 Widget、Apple Watch、灵动岛和 Universal Links 的课表同步。
- 改进错误分类、反馈入口、缓存复用和开发版/正式版来源标记。
