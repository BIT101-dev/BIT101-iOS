...
- 建立 `BIT101-iOSTests` Target，并把测试包接入主 Scheme 和 CI 编译门禁
- 新增 `Shared/Infrastructure`，收口页面提示、取消错误、账号隔离 Codable 存储和横向切换手势
- 为登录、课程列表和课程详情建立按场景划分的 Service 协议
- 从巨型 View 文件中拆出：
  - 成绩筛选偏好、排序规则与筛选 UI
  - 日程短信验证面板
  - 设置文本编辑与隐藏列表组件
...
- 为话廊、文章、我的主页、日程和成绩补齐按场景划分的 Service 协议
- 用 `PagedItemsState` 统一课程、文章、我的主页、话廊和评论列表的页码状态推进
- 为消息中心保留 `last_id` 游标语义，同时收口尾部判断和追加回写
- 将成绩状态机、图片查看器、空教室页、设置社区/关于页以及日程 URL/排序 helper 拆成独立文件
- 为共享分页状态补充单元测试，并保持所有测试 bundle 可编译
...
- 空教室查询与筛选页面已经移到 `FreeClassroomViews.swift`
...
- 账号、页面、外观、课表和 DDL 仍集中在一个文件里

当前状态：

- 功能很多，但大量是“原生表单 + 小块 helper”
- 重复主要来自多个编辑 sheet 和列表页
- 通用文本编辑 sheet、隐藏用户和隐藏帖子列表已经移到 `SettingsSupportViews.swift`
- 话廊设置与关于页已经移到 `SettingsCommunityViews.swift`
...
- 图片查看器已经移到 `GalleryImageViewer.swift`
