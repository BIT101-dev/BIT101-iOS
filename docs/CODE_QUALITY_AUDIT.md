...
- 成绩详情使用原生分组 List，并通过与日程相同的 CourseNavigationRequest 链路进入课程评价。
- 课表、成绩、可信成绩单的验证码表单已收口到 `AppSMSVerificationSheet`，只保留业务提交文案差异。
- 课程、话廊和消息的时间解析已收口到 `AppDateText`，并加入旧解析器禁用检查。
- `AppDateText` 的多格式解析和回退文案由基础设施单测覆盖。
- 课表缓存和发帖草稿共用 `AppFileDirectories`；保存失败不再使用空 `catch`，而是记录诊断。
...
- UI/组件检查现在共用 `Scripts/check-ui-consistency.py` 的契约表：按目录模式、页面后缀和已采用的公共组件自动发现成员，再检查同一契约的要求；`check-component-consistency.sh` 只负责入口转发，不再维护逐业务文件清单。
- 组件声明、页面契约和必要例外均集中在契约表；例外只保留消息中心 plain 列表、网络基础设施和平台专用布局等确有边界的项目。
- 规则优先检查“是否进入公共组件/语义令牌”，不为每个页面复制一套规则；新增同类页面只要落入目录/组件发现条件，就会自动继承契约。
