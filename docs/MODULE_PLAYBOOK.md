...
- BIT101 登录桥接可能失败。
- 网络错误可能被误判为凭据失效。
- 状态提交前校验账号身份和请求代际，隔离账号切换期间的异步结果。
...
- 缓存恢复与远端覆盖顺序可能错误。
- 学期切换后，刷新请求沿用用户当前选择的学期。
- 学校原始课表与手动调课规则分层保存；刷新时逐条比较规则对应的原始课程，课程一致时静默保留规则，课程变化时提示并移除对应规则。
...
- Watch 发布判断使用 watchOS destination 单独构建结果；iOS 真机构建结果用于 iPhone App 验证。
