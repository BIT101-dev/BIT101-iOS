...
- 用户负责 BIT101 iOS 客户端及其 iOS 真机装机验证。
- Android 客户端、Web 前端和服务端由对应主体负责；这些系统的源码改动、构建、安装、部署与上线动作转交对应维护主体。
- 本项目对 Android 客户端、Web 前端和服务端保留接口阅读、功能对照与问题记录范围。
- 服务端问题提交 issue 或联系服务端维护团队。
