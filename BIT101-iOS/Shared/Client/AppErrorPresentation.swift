...
final class AppErrorPresenter {
    ...
    private func presentNextIfPossible() {
        ...
        if item.allowsDiagnostics {
            ...
            if item.showsRecoveryLinks, item.allowsSchoolServiceLink {
    ...
    private func presentReportSheet(for item: AppErrorPresentation, generation: Int) {
        Task { @MainActor [weak self] in
            ...
            let controller = UIHostingController(rootView: AppErrorReportSheet(alert: item) { [weak self] in
...
struct DiagnosticRecoveryActions: View {
    ...
    var body: some View {
        ...
        .sheet(item: $reportAlert) { AppErrorReportSheet(alert: $0) }
    }
}
